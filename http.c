...
const char *http_request_line(int fd, char *reqpath, char *env, size_t *env_len, int size_of_reqpath)
{
    ...
    *sp2 = '\0';
    ...
    url_decode(reqpath, sp1, size_of_reqpath);
...
void http_serve(int fd, const char *name)
{
    ...
    getcwd(pn, sizeof(pn));
    int size_of_cwd = strlen(pn);
    setenv("DOCUMENT_ROOT", pn, 1);

    strncat(pn, name, 1024 - size_of_cwd - 1);
    split_path(pn);
...
void dir_join(char *dst, const char *dirname, const char *filename, int size_of_dst) {
    strncat(dst, dirname, size_of_dst - 1);
    int new_size;
    new_size = strlen(dst);
    if (dst[strlen(dst) - 1] != '/')
        strncat(dst, "/", size_of_dst - new_size - 1);
    new_size = strlen(dst);
    strncat(dst, filename, size_of_dst - new_size - 1);
}
...
void url_decode(char *dst, const char *src, int size_of_dst)
{
    for (int i = 0;;i++)
    {
        if (i == size_of_dst - 1)
